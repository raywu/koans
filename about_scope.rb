...
class AboutScope < EdgeCase::Koan
  ...
  def test_you_can_reference_nested_classes_using_the_scope_operator
    ...
    assert_equal true, fido.class != rover.class # class nested in different Modules are not the same class
  ...
  def test_nested_string_is_not_the_same_as_the_system_string
    assert_equal false, String == "HI".class # this is because class String had assumed current scope
  end

  def test_use_the_prefix_scope_operator_to_force_the_global_scope
    assert_equal true, ::String == "HI".class # two colons makes the scope global
  end
  ...
  def test_you_can_get_a_list_of_constants_for_any_class_or_module
    ...
    assert Object.constants.size > 3 # what does this mean?
  end
end
